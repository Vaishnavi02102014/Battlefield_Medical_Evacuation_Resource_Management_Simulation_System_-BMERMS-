...
frontend/app_pages/facilities.py imports data from; the page itself
contains no direct backend imports and no SQL, mirroring the pattern
used by frontend/services/dashboard_service.py.
...
reshape/label the results into the plain dict shapes the Facilities
page widgets expect.
 
Import bootstrap note: Streamlit only adds this app's own directory
(frontend/) to sys.path, not the project root. backend/ is a sibling of
frontend/, not a child of it, so the project root is added to sys.path
below before any backend.* import is resolved. Same bootstrap pattern
used by dashboard_service.py.
 
Average treatment duration — architectural note:
    crud.py has no function that aggregates Treatments.treatment_duration
    per facility (it exposes only per-casualty insert/complete/due-lookup
    helpers). This file makes one narrow, read-only exception to the
    "everything through crud.py" rule: it imports get_connection from
    backend.database.db (the same connection helper crud.py uses
    internally) and issues a single parameterized SELECT AVG(...) query,
    scoped to the private helper _average_treatment_hours(). No write, no
    schema change, and no other function in this file bypasses crud.py.
"""
...
from backend.database.db import get_connection  # noqa: E402  (see module docstring)
...
def _average_treatment_hours(facility_id: int) -> str:
    ...
    rather than 0 or a fabricated value.
...
def _build_facility_record(facility) -> dict:
    ...
    two stay consistent (same keys, same values) for the same facility.
    ...
    Field mapping:
        ...
        queue         <- facility.queue_length (maintained by
                          crud._recompute_facility_counts, not
                          re-queried separately here)
...
def get_kpi_summary() -> dict:
    ...
    Wraps crud.get_all_facilities() and aggregates in Python; there is no
    separate crud aggregate for facility-level KPIs (unlike the
    Dashboard's crud.get_dashboard_stats()), so this function owns that
    aggregation over the already-fetched Facility rows.
...
def get_facility_overview() -> list[dict]:
    ...
    Wraps crud.get_all_facilities() (ordered by facility_id: RAP, ADS,
    HMV, FDC) and maps each row through _build_facility_record().
...
def get_facility_detail(facility_name: str) -> Optional[dict]:
    ...
    value get_facility_overview() puts in each record's "name" field),
    then maps it through the same _build_facility_record() used by the
    overview, so detail and overview stay consistent for the same
    facility.
